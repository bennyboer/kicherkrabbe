...
public class Topic implements Aggregate {
    ...
    public static Topic init() {
        return new Topic(null, Version.zero(), null, Instant.now(), null);
    }
    ...
    public Aggregate apply(Event event, EventMetadata metadata) {
        TopicId id = TopicId.of(metadata.getAggregateId().getValue());
