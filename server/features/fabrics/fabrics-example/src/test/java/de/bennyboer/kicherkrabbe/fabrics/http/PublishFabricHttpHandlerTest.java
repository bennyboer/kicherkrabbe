...
public class PublishFabricHttpHandlerTest extends HttpHandlerTest {

    @Test
    void shouldSuccessfullyPublishFabric() {
        // given: a valid token for a user
        var token = createTokenForUser("USER_ID");

        // and: the module is configured to return a successful response
        when(module.publishFabric(
                "FABRIC_ID",
                3L,
        ...
        var exchange = client.post()
                .uri("/api/fabrics/FABRIC_ID/publish?version=3")
        ...
        exchange.expectBody(PublishFabricResponse.class)
    ...
    @Test
    void shouldNotAllowPublishingFabricWithoutVersionQueryParam() {
        // given: a valid token for a user
        var token = createTokenForUser("USER_ID");

        // when: posting the request without a version
        var exchange = client.post()
                .uri("/api/fabrics/FABRIC_ID/publish")
                .headers(headers -> headers.setBearerAuth(token))
                .exchange();
