...
import static de.bennyboer.kicherkrabbe.mailbox.Actions.MARK_AS_READ;
import static org.assertj.core.api.Assertions.assertThat;
import static org.mockito.ArgumentMatchers.eq;
import static org.mockito.Mockito.when;

public class MarkMailAsReadHttpHandlerTest extends HttpHandlerTest {

    @Test
    void shouldSuccessfullyMarkMailAsRead() {
        // given: a valid token for a user
        var token = createTokenForUser("USER_ID");

        // and: a request to mark the mail as read
        var request = new MarkMailAsReadRequest();
        request.version = 2L;

        // and: the module is configured to return a successful response
        var response = new MarkMailAsReadResponse();
        response.version = 3L;

        when(module.markMailAsRead(
                eq("SOME_MAIL_ID"),
                eq(request),
