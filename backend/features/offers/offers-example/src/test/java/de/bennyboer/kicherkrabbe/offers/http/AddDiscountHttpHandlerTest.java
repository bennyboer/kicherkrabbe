...
import de.bennyboer.kicherkrabbe.offers.api.requests.AddDiscountRequest;
import de.bennyboer.kicherkrabbe.offers.api.responses.AddDiscountResponse;
...
public class AddDiscountHttpHandlerTest extends HttpHandlerTest {

    @Test
    void shouldSuccessfullyAddDiscount() {
        var request = new AddDiscountRequest();
        request.version = 0L;
        var discountedPrice = new MoneyDTO();
        discountedPrice.amount = 1499L;
        discountedPrice.currency = "EUR";
        request.discountedPrice = discountedPrice;

        var token = createTokenForUser("USER_ID");

        when(module.addOfferDiscount(
                eq("OFFER_ID"),
                any(Long.class),
                any(MoneyDTO.class),
        ...
        var exchange = client.post()
                .uri("/offers/OFFER_ID/discount/add")
                .bodyValue(request)
                .headers(headers -> headers.setBearerAuth(token))
                .exchange();

        exchange.expectStatus().isOk();
        var response = exchange.expectBody(AddDiscountResponse.class)
                .returnResult()
                .getResponseBody();
    ...
    void shouldRespondWithForbiddenForMissingPermission() {
        ...
        when(module.addOfferDiscount(
                ...
                Permission.builder()
                        ...
                        .isAllowedTo(Actions.ADD_DISCOUNT)
