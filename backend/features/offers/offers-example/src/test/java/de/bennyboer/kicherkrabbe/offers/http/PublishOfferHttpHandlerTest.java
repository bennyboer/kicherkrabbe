...
import de.bennyboer.kicherkrabbe.offers.api.responses.PublishOfferResponse;
...
public class PublishOfferHttpHandlerTest extends HttpHandlerTest {

    @Test
    void shouldSuccessfullyPublishOffer() {
        var token = createTokenForUser("USER_ID");

        when(module.publishOffer(
                "OFFER_ID",
                0L,
        ...
        var exchange = client.post()
                .uri("/offers/OFFER_ID/publish?version=0")
                .headers(headers -> headers.setBearerAuth(token))
                .exchange();

        exchange.expectStatus().isOk();
        var response = exchange.expectBody(PublishOfferResponse.class)
                .returnResult()
                .getResponseBody();
    ...
    @Test
    void shouldRespondWithPreconditionFailedOnAlreadyPublished() {
        var token = createTokenForUser("USER_ID");
        ...
        var exchange = client.post()
                .uri("/offers/OFFER_ID/publish?version=1")
                .headers(headers -> headers.setBearerAuth(token))
                .exchange();
