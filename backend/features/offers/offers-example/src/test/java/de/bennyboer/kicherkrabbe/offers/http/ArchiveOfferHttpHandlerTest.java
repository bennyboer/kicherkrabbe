...
import de.bennyboer.kicherkrabbe.offers.api.responses.ArchiveOfferResponse;
...
public class ArchiveOfferHttpHandlerTest extends HttpHandlerTest {

    @Test
    void shouldSuccessfullyArchiveOffer() {
        var token = createTokenForUser("USER_ID");

        when(module.archiveOffer(
                "OFFER_ID",
                0L,
        ...
        var exchange = client.post()
                .uri("/offers/OFFER_ID/archive?version=0")
                .headers(headers -> headers.setBearerAuth(token))
                .exchange();

        exchange.expectStatus().isOk();
        var response = exchange.expectBody(ArchiveOfferResponse.class)
                .returnResult()
                .getResponseBody();
    ...
    @Test
    void shouldRespondWithPreconditionFailedOnNotReservedForArchive() {
        var token = createTokenForUser("USER_ID");
        ...
        when(module.archiveOffer(
                ...
                1L,
                Agent.user(AgentId.of("USER_ID"))
        )).thenReturn(Mono.error(new NotReservedForArchiveError()));

        var exchange = client.post()
                .uri("/offers/OFFER_ID/archive?version=1")
                .headers(headers -> headers.setBearerAuth(token))
                .exchange();
