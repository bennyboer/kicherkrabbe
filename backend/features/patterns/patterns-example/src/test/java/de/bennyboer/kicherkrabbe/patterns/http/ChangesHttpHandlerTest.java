...
public class ChangesHttpHandlerTest extends HttpHandlerTest {
    ...
    @Test
    void shouldSuccessfullyGetPatternChanges() {
        // given: a valid token for a user
        var token = createTokenForUser("USER_ID");

        // and: the module is configured to return a successful response
        when(module.getPatternChanges(
                Agent.user(AgentId.of("USER_ID"))
        )).thenReturn(Flux.just(
                ResourceChange.of(PERMISSIONS_ADDED, Set.of(ResourceId.of("PATTERN_ID")), Map.of()),
                ResourceChange.of(PERMISSIONS_REMOVED, Set.of(ResourceId.of("PATTERN_ID")), Map.of())
        ));

        // when: posting the request
        var exchange = client.get()
                .uri("/api/patterns/changes")
        ...
        assertThat(events).containsExactly(
                "{\"affected\":[\"PATTERN_ID\"],\"payload\":{},\"type\":\"PERMISSIONS_ADDED\"}",
                "{\"affected\":[\"PATTERN_ID\"],\"payload\":{},\"type\":\"PERMISSIONS_REMOVED\"}"
        );
    }
